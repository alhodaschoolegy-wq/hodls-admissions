...
import { json, checkRateLimit, getClientIp, clean } from "../utils/security.js";
...
export class ApplicationController {
  static async submit(req) {
    ...
    if (!result.success) {
      ...
      return json({
        success: false,
        message: result.errors?.[0] || "يرجى مراجعة البيانات والتأكد من صحتها.",
  ...
  static async getStatus(req) {
    ...
    if (!rate.allowed) {
      return json({ found: false, message: "يرجى التمهل في إجراء عمليات البحث." }, 429);
    }
    ...
    if (!app) {
      return json({
        found: false,
        message: "لم يتم العثور على طلب مسجل بهذه البيانات.",
      });
    }
